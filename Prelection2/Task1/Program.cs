...
string PrintNumbers3(int n) // количество операций меньше на 2
{
  string output = "0";
  for (int i = 1; i <= n; i++)
  {
...
// всего - array - n = 10
// подмассив = m = 3(по 3 элемента)
// 1 2 3 4 5 2 3 5 6 
// допустим количество элементов n-m+1
...
int[] GetRangeSum1(int[] array, int m) // сумма ренжей основной метод
{
  int n = array.Length;
  int[] t = new int[n - m + 1];
...
int[] numbers = CreateArray(5); //массив с количеством элементов
Fill(ref numbers); // ref меняем метод
Console.WriteLine(Print(numbers));

int count = 2; // количество элементов подмассива(сумму которых надо посчитать)
